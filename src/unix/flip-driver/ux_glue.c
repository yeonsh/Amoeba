/*	@(#)ux_glue.c	1.4	94/04/07 14:02:49 */
/*
...
void badcompare(file, line, c1, c2)
char *file;
{
...
void badassertionN(n, file, line)
char *file;
{
...
/*ARGSUSED*/
char *aalloc(size, align)
    vir_bytes size;
    int align;
{
    char *m;

    m = (char *) kmem_alloc(size);
    if(m != 0) bzero(m, size);
    return(m);
}
