/*	@(#)ux_glue.c	1.1	96/02/27 11:49:51 */
#include <assert.h>
INIT_ASSERT
...
**	pointer.
*/
char *	doprnt(char *begin, char *end, register char *fmt, va_list argp);

/*VARARGS3*/
char *
bprintf(char *begin, char *end, char *fmt, ...)
{
    char *rc;
    VA_LIST args;
...
static char *
put_char(char *p, char *end, char c)
{
    if (end == 0)
    {
	printf("%c", c);
...
static char *
printnum(char *begin, char *end, long int n, int base, int sign, int width, int pad)
{
    register short	i;
    register short	mod;
    char		a[MAXWIDTH];
...
/*VARARGS3*/
char *
doprnt(char *begin, char *end, register char *fmt, va_list argp)
      			      	/* pointer to buffer where to put output */
      			    	/* end of buffer to receive output */
               		    	/* format of output */
...
void badcompare(char *file, int line, long int c1, long int c2)
{
	printf("FLIP compare failed in %s, line %d: c1= %d, c2= %d\n", file,
		line, c1, c2);
	rpc_dump(0,0);
}


void badassertionN(int n, char *file, int line)
{
	printf("FLIP assertion %d failed in %s, line %d\n", n, file, line);
	rpc_dump(0,0);
}
#endif
...
/*ARGSUSED*/
char *aalloc(vir_bytes size, int align)
{
    char *m;

    m = (char *) kmem_alloc(size, KM_NOSLEEP);
    if(m != 0) bzero(m, size);
    return(m);
}
