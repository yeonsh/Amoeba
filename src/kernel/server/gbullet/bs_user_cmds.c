/*	@(#)bs_user_cmds.c	1.1	96/02/27 14:07:00 */
/*
...
b_fsize		size;	/* #bytes of data in buf */
{
    ...
    err = validate_cap(&hdr->h_priv, BS_RGT_CREATE, &inode, L_READ,BS_PUBLIC);
    if (err == STD_OK)
    {
        if ((Inode_table + inode)->i_flags&I_INUSE) /* it is committed */
	    comparing = 1;
	unlock_inode(inode);
    }
    else
    {
	if (err == STD_COMBAD)
	    err = bs_supercap(&hdr->h_priv, BS_RGT_CREATE,BS_PUBLIC);
	if (err != STD_OK)
	    return err;
    }

    /* Get an inode with new random number plus cache slot */
    err= bs_new_inode(&new) ;
 ...
 *	it first makes an uncommitted copy of the file.  It then deletes
...
b_fsize		size;		/* #bytes data in reqbuf */
{
    ...
    /* Move top piece up */
    /*  Note that i_size has been increased already. */
...
int		peer;		/* type of port for request */
{
    ...
    /* Read the file from disk or fellow member if it isn't in core */
    if (cp->c_addr == 0 && cp->c_inode.i_size > 0 ) {
	/* We are now convinced that the request is legitimate. If we do not
	 * have the file the choice is between getting its contents (wholly)
	 * and satify the request ourselves or defering the request to
	 * a member that has the file. If this is the first time, or the file
	 * size is rather big, defer the request. Otherwise get the contents.
	 * We could even store these contents on our own disk. Let this be
	 * for the moment.
	 */

	dbmessage(14,("size >0 at read"));
	if ( cp->c_inode.i_flags&I_PRESENT ) {
	    dbmessage(14,("calling loadfile"));
	    err = bs_loadfile(cp,(peer==BS_PUBLIC?1:0)) ;
	} else {
	    if ( peer==BS_PEER ) {
		unlock_inode(cp->c_innum);
		hdr->h_size = 0;
	    ...
	    dbmessage(6,("Inode %ld, used %s",cp->c_innum,
				cp->c_flags&USEDONCE?"once":"twice")) ; 
	    if (  (cp->c_flags&USEDONCE) ||
		  cp->c_inode.i_size>MAXLOCALCACHE ){
		/* Defer to member that has contents */
		err= bs_grp_defer(cp->c_innum,(Inode *)&cp->c_inode,1) ;
	    } else {
		/* Loadfile also reads from other members */
		err = bs_loadfile(cp,1) ;
	    }
	}
	if ( err!=STD_OK ) {
	    unlock_inode(cp->c_innum);
	    hdr->h_size = 0;
	    hdr->h_status = err;
	    dbmessage(4,("crapping out on read with %d",err));
	    goto CLEANUP;
	}
    }
    ...
    hdr->h_status = STD_OK;
    dbmessage(14,("Read %d bytes at offset %d from inode %d",
		hdr->h_size,hdr->h_offset,cp->c_innum));
#ifdef USE_AM6_RPC
...
CLEANUP:
    ...
    if ( hdr->h_status==(command)BULLET_LAST_ERR ) {
        dbmessage(14,("read has been answered"));
	return ;
    }
    dbmessage(14,("quitting on read with %d",hdr->h_status));
...
header *        hdr;
{
    ...
    Stats.b_size++;
    err = validate_cap(&hdr->h_priv, BS_RGT_READ, &inode, L_READ, BS_PUBLIC);
    if (err == STD_OK)
    {
