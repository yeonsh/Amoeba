 ...
 *		Ed Keizer   15-06-93
 *		Keeps a counter reflecting the number of free resources.
 *		The counter is used for purposes external to this module.
 */
...
Res_id	rid;
{
    ...
    { Res_size notu ;
      notu= a_notused(rid) ;
      if ( notu!=total ) {
	bwarn("Free count of resource %d differs: n_free %ld, notused %ld",
		rid, total, notu ) ;
      }
    }
#endif
...
Res_id	rid;
{
    ...
    rp = &a_resource[rid];
    if (!rp->r_in_use)
	message("a_dump: Free-list not initialised for RID %d\n", rid);
    else
    {
	message("a_dump: Dumping free-list for RID %d\n", rid);
	for (ap = rp->r_holelist; ap; ap = ap->h_next)
	    message("address: 0x%08x  to 0x%08x. size: 0x%x\n",
			ap->h_addr, ap->h_addr + ap->h_size, ap->h_size);
	message("a_dump: end of free list for RID %d\n", rid);
    }
    ENABLE_PREEMPTION;
}
#endif /* NDEBUG */
