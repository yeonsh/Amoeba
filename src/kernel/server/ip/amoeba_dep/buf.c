...
#undef ALLOC_BUF
		{
				...
				i++)
			{
				...
				}
				{
					acc_t *acc;
					j= 0;
					for(acc= buf512_freelist; acc;
						acc= acc->acc_next)
					{
						j++;
					}
					printf(
				"# of free 512-bytes buffer is now %d\n",
						j);
				}
			}
...
acc_t *acc;
{
	...
	for (i= 0; i<ACC_NR && acc; i++, acc= acc->acc_next)
	{
		if (acc->acc_linkC <= 0)
		{
			printf("wrong acc_linkC (%d) for acc 0x%x\n", 
				acc->acc_linkC, acc);
			return 0;
		}
		if (acc->acc_offset < 0)
		{
			printf("wrong acc_offset (%d) for acc 0x%x\n",
				acc->acc_offset, acc);
			return 0;
		}
		if (acc->acc_length < 0)
		{
			printf("wrong acc_length (%d) for acc 0x%x\n",
				acc->acc_length, acc);
			return 0;
		}
		...
		if (buffer == NULL)
		{
			printf("no buffer for acc 0x%x\n", acc);
			return 0;
		}
		if (buffer->buf_linkC <= 0)
		{
			printf(
			"wrong buf_linkC (%d) for buffer 0x%x, from acc 0x%x\n",
				buffer->buf_linkC, buffer, acc);
			return 0;
		}
		if (acc->acc_offset + acc->acc_length > buffer->buf_size)
		{
			printf("%d + %d > %d for buffer 0x%x, and acc 0x%x\n",
...
/*
 * $PchHeader: /mount/sd1a/src/minix/sys/inet/RCS/buf.c,v 1.7 1995/08/17 07:26:55 philip Exp philip $
 */
