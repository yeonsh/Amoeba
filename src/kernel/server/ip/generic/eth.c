...
FORWARD void hash_type ARGS(( eth_fd_t *eth_fd ));
FORWARD void unhash_type ARGS(( eth_fd_t *eth_fd ));
...
put_pkt_t put_pkt;
{
	...
	DBLOCK(2, printf("eth_open (%d, %d, %lx, %lx)\n", port, srfd, 
...
unsigned long req;
{
	...
	DBLOCK(2, printf("eth_ioctl (%d, %lu)\n", fd, req));
...
eth_fd_t *eth_fd;
{
	...
	{
		hash= eth_fd->ef_ethopt.nweo_type & (ETH_TYPE_HASH_NR-1);
		eth_fd->ef_type_next= eth_port->etp_type[hash];
		eth_port->etp_type[hash]= eth_fd;
	}
}

PRIVATE void unhash_type(eth_fd)
eth_fd_t *eth_fd;
{
	eth_port_t *eth_port;
...
size_t pack_size;
{
	...
	else
	{
		...
		{
			DBLOCK(0x2, printf("dropping packet for proto 0x%x\n",
				ntohs(type)));
		}			
		bf_afree(pack);
	}
...
/*
 * $PchId: eth.c,v 1.8 1995/11/21 06:45:27 philip Exp $
 */
