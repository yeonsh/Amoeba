...
PUBLIC void tcp_init()
{
	...
	for (i=0, tcp_port= tcp_port_table; i<TCP_PORT_NR; i++, tcp_port++)
	{
		if (i == tcp_conf_nr)
		{
			printf("tcp_init: more ports than configurations\n");
			break;
		}
...
tcp_port_t *tcp_port;
{
	...
	case TPS_MAIN:
		...
		tcp_conn->tc_snd_cinc=
			TCP_DEF_MSS*TCP_DEF_MSS/TCP_MAX_SND_WND_SIZE+1;
...
/*
 * $PchId: tcp.c,v 1.11 1995/11/23 11:25:56 philip Exp $
 */
