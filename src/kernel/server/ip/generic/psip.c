...
FORWARD int psip_open ARGS(( int port, int srfd,
	...
	put_pkt_t pkt_pkt ));
FORWARD int psip_ioctl ARGS(( int fd, unsigned long req ));
...
PUBLIC void psip_init()
{
	...
	for (i=0, psip_port= psip_port_table; i<PSIP_PORT_NR; i++, psip_port++)
	{
		if (i == psip_conf_nr)
		{
			printf("psip_init: more ports than configurations\n");
			break;
		}
...
/*
 * $PchId: psip.c,v 1.5 1995/11/21 06:45:27 philip Exp $
 */
