...
udp_port_t *udp_port;
{
	...
	case UPS_EMPTY:
		...
		udp_port->up_ipfd= ip_open(udp_port->up_ipdev, 
			udp_port-udp_port_table, udp_get_data, udp_put_data, 0);
...
acc_t *pack;
{
	...
	size_t udp_size, data_size, opt_size;
...
assert (pack->acc_length >= IP_MIN_HDR_SIZE);
	...
	for (i=0, udp_fd=udp_fd_table; i<UDP_FD_NR; i++, udp_fd++)
	{
		...
		if (!(udp_fd->uf_flags & UFF_OPTSET))
		{
			DBLOCK(0x10, printf("%d: options not set\n", i));
		...
		{
			DBLOCK(1, printf(
			"%d: wrong rem port, I got %d, expected %d\n",
				i, ntohs(src_port),
				ntohs(udp_fd->uf_udpopt.nwuo_remport)));
		...
		{
			DBLOCK(1, printf("%d: wrong rem addr\n", i));
			...
			if (!ipopt_pack)
			{
				...
				*(udp_io_hdr_t *)ptr2acc_data(ipopt_pack)=
				...
				ipopt_pack->acc_next= bf_cut(ip_acc,
					(size_t)IP_MIN_HDR_SIZE, opt_size);
...
assert (tmp_acc->acc_linkC == 1);
		...
		if (udp_fd->uf_rdbuf_head &&
			(flags & NWUO_ACC_MASK) == NWUO_SHARED)
		{
			share_fd= udp_fd;
			continue;
		}
...
/*
 * $PchId: udp.c,v 1.7 1995/11/21 06:51:58 philip Exp $
 */
