/*	@(#)ip_read.c	1.4	96/02/27 14:17:37 */
/*
ip_read.c
...
ip_hdr_t *ip_hdr;
{
	...
	else
	{
		...
		{
			DBLOCK(0x4, printf("dropping packet for proto %d\n",
				proto));
		}
		bf_afree(pack);
	}
...
assert (pack->acc_length >= IP_MIN_HDR_SIZE);
	...
	DIFBLOCK(4, (ip_hdr->ih_dst & HTONL(0xf0000000)) == HTONL(0xe0000000),
	...
	{
		printf("broadcast packet for ip-nonbroadcast addr, src=");
		writeIpAddr(ip_hdr->ih_src);
		printf(" dst=");
		writeIpAddr(ip_hdr->ih_dst);
		printf("\n");
...
/*
 * $PchId: ip_read.c,v 1.7 1995/11/21 06:45:27 philip Exp $
 */
