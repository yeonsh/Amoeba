/*	@(#)tcp_recv.c	1.6	96/02/27 14:19:06 */
/*
tcp_recv.c
...
PRIVATE void process_data(tcp_conn, tcp_hdr, tcp_data, data_len)
tcp_conn_t *tcp_conn;
tcp_hdr_t *tcp_hdr;
acc_t *tcp_data;
int data_len;
...
PUBLIC void tcp_fd_read(tcp_conn, enq)
tcp_conn_t *tcp_conn;
int enq;					/* Enqueue writes. */
{
	...
	if (fin_recv || !tcp_fd->tf_read_count)
	{
		tcp_reply_read (tcp_fd, tcp_fd->tf_read_offset);
		return;
	}
...
/*
 * $PchId: tcp_recv.c,v 1.10 1995/11/21 09:01:01 philip Exp $
 */
