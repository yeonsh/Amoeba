...
cache_init()
{
    ...
    if (i < BS_MIN_MEMORY)
    {
	printf("Only 0x%lx bytes cache available.  Minimum needed is 0x%lx\n",
							    i, BS_MIN_MEMORY);
...
Inodenum	inode;
{
    ...
    {
	/*
	 * Last slot on lru is in use!
	 * Look for a committed, unlocked file to evict from the cache.
	 */
	DISABLE_PREEMPTION;
	while (cp != (Cache_ent *) 0 &&
		((cp->c_flags & NOTCOMMITTED) || inode_locked(cp->c_innum)))
	{
	    cp = cp->c_junior;
	}

	if (cp == (Cache_ent *) 0)	/* Eeek! No uncommitted slot */
	{
	    ENABLE_PREEMPTION;
	    mu_unlock(&Cache_sema);
	...
	else /* evict cp from the cache */
	{
	    Inodenum	deadi;

	    deadi = cp->c_innum;

	    /* Note that deadi is guaranteed not to be locked */
	    mu_unlock(&Cache_sema);
	    /* We have to lock it exclusively while we trash the cache entry. */
	    lock_inode(deadi, L_WRITE);
	    ENABLE_PREEMPTION;
...
register b_fsize	size;
{
	...
	DISABLE_PREEMPTION;
	/*
	 * Because no one can preempt, if we see that the lock is free
	 * then we are sure of getting the lock.  Don't add anything
	 * that can block between the disable and the enable!
	 */
	/* If cache slot is in use and we can throw away its data */
	nextcp = cp->c_junior;
	if (cp->c_innum != 0 && !inode_locked(cp->c_innum) &&
			!(cp->c_flags & NOTCOMMITTED) && cp->c_bufsize != 0)
	{
	    ...
	    ENABLE_PREEMPTION;
	    assert(!(cp->c_flags & NOTCOMMITTED));
	    /*
	     * Alas, scrounging can take a long time - too long for the
	     * interrupt queue so we have to take a risk and let other
	     * things run.  After every 64 cached inodes we take a
	     * threadswitch.
	     */
	    if ((i_count & 63) == 0)
		threadswitch();
 ...
 *	NB: It assumes that the caller has not locked the cache!
 */
...
RETRY:
    ...
    if ((err = validate_cap(priv, min_rights, &inode, locktype)) != STD_OK)
	return err;
...
#ifndef NDEBUG
    if ((Inode_table + inode)->i_lifetime == 0) /* uncommitted file */
...
b_fsize	size;
{
	...
	{
	    scrounge_memory(size);	/* try to get the memory */
	    if ((result = a_alloc(A_CACHE_MEM, (Res_size) size)) == 0)
	    {
...
register b_fsize	size;	/* # bytes to allocate */
{
	...
	if ((cp->c_addr = alloc_buffer(size)) == 0)
	    return STD_NOMEM;
    cp->c_bufsize = size;
 ...
 *	c) uncommitted files
 *	d) internal fragmentation in uncommitted files
...
void
cache_usage_stats(committed, comm_frag, uncommitted, uncomm_frag, slots)
...
#endif
{
	...
	for (cp = Cachetab, i = 0; i < Num_cache_slots; cp++, i++)
	{
	    ...
	    {
		/* Cache slots has something in it! */
		...
		if (cp->c_innum == inode) /* We got it */
		{
			...
			if (--cp->c_timeout == 0)
			{
			    destroy_inode(inode); /* unlocks the inode */
			    Stats.i_uncomm_tout++;
			}
			else
			    unlock_inode(inode);
...
cache_check()
{
    ...
    {
	printf("Bullet Server: Youngest = 0x%lx, Oldest 0x%lx\n",
							    Youngest, Oldest);
...
Cache_ent *	cp;
{
    printf("CACHE ENTRY: junior 0x%x, senior 0x%x, hnext 0x%x, hprev 0x%x\n",
			cp->c_junior, cp->c_senior, cp->c_hnext, cp->c_hprev);
    printf("  addr 0x%x, bufsize 0x%x, innum 0x%x, flags 0x%x, timeout 0x%x\n",
	    cp->c_addr, cp->c_bufsize, cp->c_innum, cp->c_flags, cp->c_timeout);
    printf("CACHED ");
    dump_inode(&cp->c_inode);
    printf("REAL ");
...
Inodenum inum;
{
    ...
    printf("dump_cache_for_inode: inode 0x%x not in cache\n", inum);
    return 0;
}
...
Inode *	ip;
{
    ...
    printf("INODE: i_dskaddr 0x%x, i_size 0x%x\n", addr, size);
    printf("       i_random %p, i_lifetime 0x%x, i_flags 0x%x\n",
				&ip->i_random, ip->i_lifetime, ip->i_flags);
}
