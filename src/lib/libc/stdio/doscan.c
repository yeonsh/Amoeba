/*	@(#)doscan.c	1.4	96/03/15 14:06:36 */
/*
...
/* if sizeof(pointer) != sizeof(int), assume it has the size of long */
#define set_pointer(flags) \
    ((sizeof(int) == sizeof(char *)) ? 0 : (flags |= FL_LONG))
 ...
 */
static char *
o_collect(c, stream, type, width, basep)
register int c;
		...
		case 'G':
			...
			if (!(flags & FL_NOASSIGN)) {
				...
				if (flags & FL_LONGDOUBLE)
					*va_arg(ap, LONG_DOUBLE *) = (LONG_DOUBLE) ld_val;
				else
				    if (flags & FL_LONG)
