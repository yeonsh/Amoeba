/*	@(#)ext_comp.c	1.3	96/02/29 16:46:47 */
#ifndef NOFLOAT
/*
  (c) copyright 1996 by the Vrije Universiteit, Amsterdam, The Netherlands.
...
static int b64_add _ARGS((struct mantissa *e1, struct mantissa *e2));
static void b64_sft _ARGS((struct mantissa *e1, int n));
...
struct EXTEND *e;
{
	...
	*/
	...
	if (max_d.exp == 0) {
#ifdef __GNUC__
		/* The conversion of DBL_MAX fails when using softfp and the
		 * floatlib package;  probably because it only works in single
		 * precision.
		 */
		_dbl_ext_cvt(FLT_MAX, &max_d);
		max_d.exp = DBL_MAX_EXP;
#else
