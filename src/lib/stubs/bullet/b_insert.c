/*	@(#)b_insert.c	1.2	94/04/07 10:59:45 */
/*
...
** b_insert
**
**	The bullet server "insert" client stub.
**	If the data in buf won't fit in one transaction then we have do several
**	insert commands.
...
errstat
b_insert(cap, offset, buf, size, commit, newfile)
...
capability *	newfile;	/* out: capability for created file */
{
    ...
    hdr.h_priv = cap->cap_priv;		/* Initial cap. May be changed by BS */
    hdr.h_offset = offset;
    do
    {
	hdr.h_command = BS_INSERT;
	...
	sendsize = hdr.h_size; /* save this since server changes h_size */
