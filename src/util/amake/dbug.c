...
EXPORT BOOL _db_on_ = FALSE;		/* TRUE if debugging currently on */
EXPORT BOOL _db_pon_ = FALSE;	/* TRUE if debugging currently on */

/*
 *	Externally supplied functions.
 */

#if (amunix || xenix)		/* Only needed for unix */
...
IMPORT int exit ();		/* Terminate execution */
...
#else
IMPORT int doprnt ();		/* Varargs form of fprintf */
#endif

IMPORT int free ();
/* KV: changed "VOID free()" to "int free()" to keep lint happy */
...
#ifndef MIPSEB
/* XXXX This might conflict with string.h usage */
...
LOCAL BOOL init_done = FALSE;/* Set to TRUE when initialization done */
LOCAL char **framep = NULL;	/* Pointer to current frame */

#if (amunix || xenix || amiga || AtariSt)
LOCAL int jmplevel,catchlevel;	/* Remember nesting level at setjmp () */
LOCAL char *jmpfunc,*catchfunc;	/* Remember current function for setjmp */
LOCAL char *jmpfile,*catchfile;	/* Remember current file for setjmp */
LOCAL char **jmpframep, **catchframep;
...
LOCAL BOOL DoTrace ();	/* Test for tracing enabled */
LOCAL BOOL Writable ();	/* Test to see if file is writable */
...
#else
IMPORT char *strtok ();		/* Break string into tokens */
#endif
...
#else
	(VOID) doprnt (_db_fp_, format, args);
#endif
...
/*VARARGS1*/
VOID _db_write_ (format, va_alist)
char *format;
va_dcl
{
    va_list args;
...
FILE *fp;
{
    if (fp != (FILE *)NULL && fp != stderr && fp != stdout) {
	if (fclose (fp) == EOF) {
...
#endif	/* !amunix && !xenix && !(amiga && LATTICE) */
...
LOCAL unsigned long Clock ()
{
    ...
    /* clock() delivers # of clock ticks since system startup.
     * A second contains CLK_TCK clicks.
     * So the number of miliseconds since the last call to Clock() is
     *        number_of_ticks_elapsed * number_of_miliseconds_per_tick ==
     *              (clock() - begin) * (1000 / CLK_TCK)
     * The first call to Clock() will almost certainly deliver 0, so call
     * it on startup if you want something close to (sys+user time).
     */
     return ((clock() - begin) * (1000 / CLK_TCK));
}
#endif /* AtariSt */
