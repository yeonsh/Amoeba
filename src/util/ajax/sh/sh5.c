/*	@(#)sh5.c	1.1	91/04/12 12:39:17 */
#define Extern extern
#include <sys/types.h>
#include <signal.h>
#include <errno.h>
#include <setjmp.h>
#include <fcntl.h>	/* For remap */
#include "sh.h"
	...
	c = readc();
 	if (ec != '\'' && ec != '`' && e.iop->task != XGRAVE) {
		if(c == '\\') {
			...
			if (c == '\n') {
				/*
				** Print PS2 when using \'s in commands...
				**
				** peterb@cwi.nl
				*/
				...
				/* backslash within double quotes is special:
				 * if the character following it is not one of
				 * \ " ` or $ then the \ itself is quoted.
				 * The char following it is returned in the
				 * next call to getc().
				 */
				if (ec == '"' && !any(c, "\\\"`$")) {
					unget(c);
...
/*
 * remap fd into Shell's fd space, if it is less then e.iofd.  Preserve its
 * close-on-exec bit.
 */
int
remap(fd)
register int fd;
{
	register int i;
	int map[NOFILE];
	/* Make sure that all fds under e.iofd are open (with close-on-exec
	 * flag set), so that we only need to do one dup to obtain an
	 * fd >= e.iofd: */
	...
	if (first_time) {
		...
		if (i < 0) {
			err("cannot open close-on-exec fd's for remap");
		} else
			close(i);
#ifdef DEBUG
		prs("remap: opened "); prn(i); prs(" fd's to /dev/null\n");
#endif
	}
	/* Propagate errors in open/creat correctly: */
	if (fd < 0) {
#ifdef DEBUG
		prs("remap->"); prn(fd); prs("\n");
#endif
		first_time = 0;
		return(fd);
	}
	...
	if (fd < e.iofd) {
		/* After the first call to remap, which initializes all user
		 * fds, above, this code should only be reached when one of
		 * the original three fd's is being mapped to a high fd, in
		 * order to save it (fds 0, 1, or 2).
		 */
#ifdef DEBUG
		prs("remap: got user fd "); prn(fd); prs("; dup'ing it\n");
#endif
		for (i = 0; i < e.iofd; i++)
			map[i] = 0;
		do {
		...
		for (i = 0; i < e.iofd; i++)
			if (map[i]) {
				int j;
				close(i);
				/* Restore it to a dup'd copy of /dev/null: */
				do {
					j = open("/dev/null", 0);
					fcntl(j, F_SETFD, 1);
				} while (j >= 0 && j < i);
	...
	fcntl(fd, F_SETFD, 1);  /* We want all high fd's to be closed on exec */
#ifdef DEBUG
	prs("remap->"); prn(fd); prs("\n");
#endif
	first_time = 0;
