...
register char *pp;
{
	...
	for (dp = cp = space((int)(pp-np)+3); np < pp;)
		*cp++ = *np++;
#if 0 /* Most Amoeba directories don't (yet) have an entry '.' */
	*cp++ = '.';
#endif
	*cp = '\0';
	...
	*cp = '\0';
	...
	dname[NAME_MAX] = '\0';
	while ((entp = readdir(dn)) != NULL) {
		strcpy(dname, entp->d_name);
		if (dname[0] == '.')
			if (*gp != '.')
				continue;
		for(k=0; dname[k] != '\0'; k++)
