...
create_file_names()
{
    ...
    (void) strcpy(action_file_name, tmpdir);
    (void) strcpy(text_file_name, tmpdir);
    (void) strcpy(union_file_name, tmpdir);
    ...
    (void) strcpy(action_file_name + len, temp_form);
    (void) strcpy(text_file_name + len, temp_form);
    (void) strcpy(union_file_name + len, temp_form);
	...
	(void) strcpy(defines_file_name, prefix);
	(void) strcpy(defines_file_name + len, DEFINES_SUFFIX);
    }

    output_file_name = (char *) MALLOC(len + 7);
    if (output_file_name == 0) no_space();
    (void) strcpy(output_file_name, prefix);
    (void) strcpy(output_file_name + len, OUTPUT_SUFFIX);

    if (vflag)
    {
	verbose_file_name = (char *) MALLOC(len + 8);
	if (verbose_file_name == 0) no_space();
	(void) strcpy(verbose_file_name, prefix);
	(void) strcpy(verbose_file_name + len, VERBOSE_SUFFIX);
    }
}
