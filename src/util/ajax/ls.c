 ...
 */
#ifndef lint
static char version[] = "ls version 1.1 07/13/89";
#endif /* lint */
...
#ifndef major
/* Posix doesn't support major and minor */
#define major(x) 0
#define minor(x) 0
#endif
...
/*@ \fILs\fP takes quite a few options:
 * .IP \fB\-l\fP
...
int	flags_l,	flags_g,	flags_t,	flags_a,
	flags_A,	flags_s,	flags_d,	flags_r,
	flags_u,	flags_c,	flags_i,	flags_f,
	flags_F,	flags_R,	flags_C;
...
int argc; char *argv[];
{
 ...
 if(flags_l || flags_s || flags_i)
    flags_C = 0;

 flags_r = flags_r ? -1 : 1;		/* multiplier for comparisons */

 if(geteuid() == 0 && !flags_a)
    flags_A = 1;
...
*/
void add_file(/* char *filename, struct lsfile *parent, ino_t inum,
		 int savename */);
...
#define DEFARG "."
 if(argc == 1)
    add_file(DEFARG, (struct lsfile *)NULL, (ino_t) 0, 0);
 else
    while(++argv, --argc)
	 {
	  add_file(*argv, (struct lsfile *)NULL, (ino_t) 0, 0);
	 }
}
...
struct lsfile *entry;
{
 ...
 while((dp = readdir(dirp)) != NULL)
      {
       if(dp->d_name[0] == '.' && !(flags_A || flags_a))
	  continue;
       if(DOTDIR(dp->d_name) && !flags_a)
	  continue;
#ifndef AMOEBA
       add_file(dp->d_name, parent, (ino_t) dp->d_ino, 1);
#else
       add_file(dp->d_name, parent, (ino_t) 0, 1);
#endif
      }
...
#ifndef AMOEBA /* Amoeba *still supports* null pathnames */
...
struct lsfile *entry;
{
 ...
 for(q = parentname, p = name; *q; )
     *p++ = *q++;

 if(p > name && p[-1] != '/')
     *p++ = '/';

 for(q = entry->f_name; *q; )
     *p++ = *q++;
...
int include_dirs;
{
 ...
 for(i = baseindex; i < lastindex; ++i)
	...
	&& (include_dirs
	    || !S_ISDIR(sortindex[i]->f_stat.st_mode)))
	if(flags_C)
	   listcol(sortindex[i]);
	else
	   listone(sortindex[i]);
 ...
 fflush(stdout);
 if(flags_d || (baseindex > 0 && !flags_R))
    return;
 ...
 for(i = baseindex; i < lastindex; ++i)
	...
	add_dir(sortindex[i]);
	if(!flags_f)
	   sortfiles(ofilep - files, filep - ofilep);
	if(flags_l || flags_s)
	  {
	   int i;
	   int total = 0;
	   for(i = ofilep - files; i < filep - files; ++i)
	      {
	       if(!S_ISDIR(sortindex[i]->f_stat.st_mode))
...
register struct lsfile *entry;
{
 ...
 if(flags_i)
    printf("%5d ", entry->f_stat.st_ino);

 if(flags_s)
    printf("%4d ", nblocks(entry->f_stat.st_size));
 ...
 if(flags_l)
   {
    ...
    switch(mode & _S_IFMT)
	  {
	   case _S_IFDIR:	bits[0] = 'd';
				break;
	   case _S_IFBLK:	bits[0] = 'b';
				break;
	   case _S_IFCHR:	bits[0] = 'c';
				break;
	   default:		bits[0] = '-';
				break;
	  }
    ...
    if(flags_g)
       printf("%-8.8s ", groupname(entry->f_stat.st_gid));
 ...
 *colentp = stringp;
 ...
 if(flags_F && (c = fchar(entry->f_stat.st_mode)) != '\0')
    ADDSTRING(c);
...
/*@*Date and time.
 * |prdate()| prints the specified time in the format used by
...
/*@*Counting blocks.
 * This code is 'stolen' almost verbatim from Andrew S. Tannenbaum's
...
/* #ifdef AMOEBA */
/* These constants are an attempt to tune ls for Amoeba (i.e., indirect
 * blocks will not be counted in the length.  For UNIX, ls knows that a
 * multimegabyte file has plenty of indirect blocks, which it includes in
 * the ls -ls listing.
 */
/*AMOEBA - I (cbo) took this stuff out */
/* #define NR_DZONE_NUM 200000000L */
/* #define NR_INDIRECTS 200000000L */
/* #else */
/* ...???... */
/* #endif */
...
/* Convert file length to blocks, including indirect blocks. */
  ...
  blocks = fileb;
/*AMOEBA - will not have any indirect blocks */
#ifdef AMOEBA
  return(blocks);
#else
