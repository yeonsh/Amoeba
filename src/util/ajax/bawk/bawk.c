...
char **argv;
{
        ...
        (void) strcpy( Fieldsep, " \t" );
        (void) strcpy( Recordsep, "\n" );
...
/* StringPtr can temporarily be set to a buffer to read from */
static char *StringPtr = 0;
...
compile()
{
        ...
        while ( (c = getcharacter()) != -1 )
        {
                ...
                else if ( c=='{' )
                {
                        ...
                        {
                                Rulep->nextrule = (struct rule *) getmem( sizeof( *Rulep ) );
                        ...
                        if ( !Rulep )
                        {
                                 ...
                                 * initialize it
                                 */
                                Rules = Rulep = (struct rule *) getmem( sizeof( *Rulep ) );
                                fillmem( (char *) Rulep, sizeof( *Rulep ), 0 );
                        }
                        Rulep->action = getmem( (unsigned) len );
                ...
                else if ( c==',' )
                {
                        ...
                        Rulep->pattern.stop = getmem( (unsigned) len );
                ...
                else
                {
                        ...
                        Rulep->pattern.start = getmem( (unsigned) len );
                        movemem( Workbuf, Rulep->pattern.start, len );
                }
        }
        endfile();

	/* add a default "print $0" action to rules that don't have one yet */
	{
		static char printstmt[] = "{ print $0 }";
		char *default_action;
		...
		StringPtr = 0;
                ...
                default_action = getmem( (unsigned) len );
                movemem( Workbuf, default_action, len );

		for (Rulep = Rules; Rulep != 0; Rulep = Rulep->nextrule) {
			if (Rulep->action == 0) {
				Rulep->action = default_action;
			}
		}
	}
}
...
char *delim;
{
        ...
        while ( *str )
        {
                ...
                (void) strcpy( wrdlst[ wrdcnt++ ], wrdbuf );
        }
...
ungetcharacter( c )
{
	/* first check if we're reading from a string */
	if (StringPtr != 0) {
		*--StringPtr = c;
		return 0;
	}
