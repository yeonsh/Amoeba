/*	@(#)bawksym.c	1.1	91/04/11 09:48:45 */
/*
...
function( funcnum )
{
        ...
        case F_PRINT:   /* quick and simple string printing */
                pushint(pprint(args[0].dptr, args[1].dptr, args[2].dptr,
                     args[3].dptr, args[4].dptr, args[5].dptr, args[6].dptr,
 		         args[7].dptr, args[8].dptr, args[9].dptr));
                break;
        case F_PRINTF:  /* just like the real printf() function */
                pushint( printf( args[0].dptr, args[1], args[2], args[3],
		...
		endfile();
                pushint( 0 ); /* it seems we have to push something here! */
                break;
        default:        /* oops! */
...
assignment()
{
        ...
        if ( Stackptr->lvalue )
        {
                if ( Stackptr->class )
                        ...
                        movemem( (char *) &ival, Stackptr->value.dptr,
							    Stackptr->size );
                pop();
                pushint( ival );
        }
