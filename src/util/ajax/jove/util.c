...
{
#ifdef AMOEBA
	*dp = 4.0;	/* should really ask the runserver instead */
#else
#ifdef sun
	long	avenrun[3];
#else
...
int	delay;
{
	...
	redisplay();
#ifdef AMOEBA
	/* Actually, the following sleep() causes a somewhat longer delay than
	 * asked for, but who cares ..
	 */
	sleep((unsigned) (delay / 5));
#else
