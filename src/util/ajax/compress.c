/*	@(#)compress.c	1.3	96/02/27 12:44:13 */
 ...
 */
	...
	*bp = (*bp & rmask[r_off]) | (code << r_off) & lmask[r_off];
	...
	if ( free_ent > maxcode || (clear_flg > 0))
	{
	    ...
	    if ( offset > 0 ) 
		{
			if( fwrite( buf, 1, (size_t) n_bits, stdout ) != n_bits)
				writeerr();
			bytes_out += n_bits;
	    }
	    offset = 0;
	 ...
	 */
	if ( offset > 0 )
	    (void) fwrite( buf, 1, (size_t) ((offset + 7) / 8), stdout );
...
getcode() 
{
    ...
    if ( clear_flg > 0 || offset >= size || free_ent > maxcode ) 
	{
		...
		size = fread( buf, 1, (size_t) n_bits, stdin );
		if ( size <= 0 )
...
# endif /*AMOEBA*/
...
#if defined(AZTEC86) && ! defined(AMOEBA)
	...
	{			/* ***** Successful Compression ***** */
		...
		if (chmod(ofname, mode))		/* Copy modes */
		    perror(ofname);
#ifndef PCDOS
		(void) chown(ofname, statbuf.st_uid, statbuf.st_gid);	/* Copy ownership */
#if defined(AMOEBA)
