/*	@(#)display.c	1.2	96/03/19 13:03:02 */
...
display(n,nd,nlines,reallydispl)
  long n; int nd; register int nlines; int reallydispl; {
	...
	if (!stupid) {
		...
		if (reallydispl) {
			fillscr(nlines, "~\r\n");
			fillscr(maxpagesize - s->currentpos - nlines, "\r\n");
		}
	}
	return compretval(p);
}
...
scrollf(n,reallydispl) int n; int reallydispl; {
	...
	if (n <= 0 || ((status & EOFILE) && ! waiting_for_data)) return status;
