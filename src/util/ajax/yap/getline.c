/*	@(#)getline.c	1.3	96/03/19 13:03:22 */
...
getblock(n, disable_interrupt) register long n; {
	...
	if (n < lastreadline ||
	    (n == lastreadline && !(pblock->b_flags & INCOMPLETE_LINE))) {
		/*
...
nextblock(pblock) register struct block *pblock; {
	...
	for (;;) {
		...
		if (cnt < 0) {
			 ...
			 */
			if (errno == EINTR) {
				if (! waiting_for_data) continue;
			}
			else error("Could not read input file");
			cnt = 0;
		}
		else if (cnt == 0 && waiting_for_data) {
			if (! interrupt) {
				sleep(1);
				continue;
			}
	...
	*c = 0;
	if (c != pblock->b_info && *(c-1) != 0) {
		...
		*poff++ = c - pblock->b_info + 1;
		...
		else pblock->b_flags |= INCOMPLETE_LINE;
	}
	...
	if (pblock->b_flags & PARTLY) {
		...
		if (c == pblock->b_info) {
			lastreadline++;
			pblock->b_end = 0;
			pblock->b_flags |= INCOMPLETE_LINE;
		}
	}
	else {
		/*
...
# endif
	retval = read(0, &c, 1);
	if (interrupt && (retval == 0 || (retval == -1 && errno == EINTR))) {
		return -1;
	}
	if (retval == -1) panic("read error");
	return c & 0177;
}
