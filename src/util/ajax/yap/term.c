...
extern short	ospeed;		/* Needed for tputs() */
extern char	PC;		/* Needed for tputs() */
extern char *	UP;		/* Needed for tgoto() */
static char	*ll;
...
# ifdef TIOCSPGRP
	 ...
	 * on a pipe to yap)
	 */
	if (! nopipe && ioctl(0, TIOCSPGRP, (char *) &proc_id) != -1) {
#if USG_TTY || POSIX_TTY
		setpgid(0, proc_id);
#else
		setpgrp(0, proc_id);
#endif
...
ini_terminal() {
	...
	register UG, SG;
	char tempbuf[20];
	char *mb, *md, *mh, *me, *mr;	/* attributes */
