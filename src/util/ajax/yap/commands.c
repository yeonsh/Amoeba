/*	@(#)commands.c	1.3	96/03/19 13:02:53 */
...
do_search(str,cnt,dir) char *str; long cnt; int dir; {
	...
	for (;;) {
		...
		if (re_exec(p) && --cnt <= 0) {
			 ...
			 */
			if (dir > 0) {
				if (lineno <= scr_info.lastline &&
				    !(status & EOFILE)) {
					int i = - scr_info.nf;
					struct linelist *h = scr_info.head;
					while (h->line < lineno) {
						i += h->cnt;
						h = h->next;
					}
				...
				else (VOID) display(lineno, 0, pagesize, 1);
			}
			else {
				(VOID) display(lineno, 0, pagesize, 1);
			}
			return;
		}
	}
...
do_wait_for_data(cnt) long cnt; {
	...
	(VOID) scrollb(j - scr_info.tail->cnt + 1, 0);
	redraw(0);
	waiting_for_data = 1;
	give_prompt();
	flush();
	while (! interrupt) {
		if (scr_info.currentpos >= pagesize-1) {
			(VOID) scrollf(1, 1);
		}
		else {
			display(scr_info.firstline, scr_info.nf, scr_info.currentpos+1, 0);
			waiting_for_data = 0;
			redraw(0);
			waiting_for_data = 1;
		}
...
STATIC VOID shell(esc_ch, cnt) long cnt;
{
	register char *p;
	static char buf[2];
