...
#else
#ifdef AMOEBA
	/* libajax currently lacks a working utime, so we touch the
	 * the file by adding nothing to it.
	 */
	if ((fd = open(np->n_name, O_RDWR)) < 0) {
	    printf("%s: '%s' not touched - non-existant\n",
		   myname, np->n_name);
	} else {
	    close(fd);
...
#else
	if (utime(np->n_name, (time_t *)NULL) < 0)
		printf("%s: '%s' not touched - non-existant\n",
				myname, np->n_name);
#endif /* AMOEBA */
#endif /* MINIX */
...
char        **pinputname;
{
  ...
  *pbasename = (char *)0;
#else
  /* Even though we cannot find a value for "$<", we still want "$*" to be set.
   * This is needed to compile X.
   */
#endif
  return;
}
