/*	@(#)reader.c	1.1	91/04/12 09:15:31 */
/*************************************************************************
 *
...
FILE *fd;
{
  ...
  for (;;) {
	...
	for (q = p; q != NULL && *q != '\0' ;) {
	    switch (*q) {
	    case '\'':
	    case '"': /* skip till after next quote */
		if ((q = strchr(q + 1, *q)) != NULL) {
		    q++;
		}
		break;
	    case '#':
		if (p != q && q[-1] == '\\') {
		    ...
		    *q = '\0';		/* ignore text from # onward */
		}
		break;
	    case '\n':
		if (q[1] == '\0') {	/* the normal case */
		    *q = '\0';		/* will be restored to \n */
		} else {
		    q++;		/* quoted end of line, continue */
		}
		break;
	    default:
		q++;
