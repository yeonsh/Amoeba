/*	@(#)treecmp.c	1.3	96/03/01 16:53:10 */
...
/* Forward */
void compare _ARGS((char *, char *));
void regular _ARGS((char *, char *));
void directory _ARGS((char *, char *));
void check _ARGS((char *, struct dirstruct *, int, char *));
 ...
 */
  ...
  type1 = stat1.st_mode & _S_IFMT;
  type2 = stat2.st_mode & _S_IFMT;
  if (type1 != type2) {
  ...
  switch(type1) {
	case _S_IFREG:	regular(f1, f2);
			break;

	case _S_IFDIR:	directory(f1, f2);
			break;
...
/* Compare to regular files.  If they are different, complain. */
  ...
  count = stat1.st_size;
  while (count > 0L) {
	bytes = (count > BUFSIZE ? BUFSIZE : count);	/* rd count */
 ...
 */
  ...
  fd1 = open(f1, 0);
  if (fd1 > 0) n1 = read(fd1, dir1buf, (int) dir1bytes);
  ...
  fd2 = open(f2, 0);
  if (fd2 > 0) n2 = read(fd2, dir2buf, (int) dir2bytes);
