...
#ifndef lint
static char *RCSid = "$Id: emacs.c,v 1.3 1994/11/14 09:06:59 versto Exp $";
#endif
...
x_init_emacs()
{
	...
	x_tab = (struct x_ftab const*(*)[X_TABSZ])
		alloc(sizeofN(*x_tab, 3), AEDIT);
	for (j = 0; j < 128; j++)
	...
	int erase, kill, werase, intr, quit;
{
	x_tab[0][erase] = xft_erase;
        if (kill != CTRL('X')) {
                /* AMOEBA fix: Ignore ^X; it's a standard prefix! */
	...
	if (dirnam)  {
		...
		if (stat(buf, &statb)==0)
			if (S_ISDIR(statb.st_mode))
				...
				type = '*';
#else
			/* The above does work, but we avoid it here since
			 * it can be a very time consuming operation.
			 * To determine if a file is executable, a header	
			 * of the binary has to be read. Given the `whole
			 * file caching' approach of the Bullet server,
			 * this can be very costly when it has to be done
			 * for many files starting with the same character..
			 */
#endif
		if (type)
			++len;
