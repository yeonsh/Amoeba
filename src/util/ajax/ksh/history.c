/*	@(#)history.c	1.3	96/02/27 13:02:42 */
/*
 * command history
 *
...
#ifndef lint
static char *RCSid = "$Id: history.c,v 1.3 1994/11/14 09:06:59 versto Exp $";
#endif
/*
 *	This file contains
	...
	register char **wp;
{
	...
	if (sflag) {
		...
		while ((id = *wp++) != NULL) {
			...
			if ((p = strchr(id, '=')) != NULL) {
				if (p == id) /* non-empty pattern required */
				    errorf("fc: invalid replacement\n");
				pat = id;
				rep = p;
				*rep++ = '\0';
			} else
