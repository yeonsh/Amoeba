...
#ifndef lint
static char *RCSid = "$Id: edit.c,v 1.3 1994/11/14 09:06:59 versto Exp $";
#endif
...
#include "sh.h"
#ifdef AMOEBA
#define _BSD    1               /* only in this file */
#endif
#include "tty.h"
#define EXTERN
#include "edit.h"
	...
	size_t len;
{
	...
	if (i > 4 && strstr(buf, "stty"))
		x_do_init = 1;
#ifdef AMOEBA
	/* check for the Amoeba specific "sty" as well */
	if (i > 3 && strncmp(buf, "sty", 3) == 0)
		x_do_init = 1;
#endif
	if (i < 0 && errno == EINTR)
		trapsig(SIGINT);
