/*	@(#)c_ksh.c	1.3	96/02/27 13:01:22 */
/*
 * built-in Korn commands: c_*
 */

#ifndef lint
static char *RCSid = "$Id: c_ksh.c,v 1.3 1994/11/14 09:06:59 versto Exp $";
#endif
	...
	register char **wp;
{
	...
	{
		setstr(v_pwd, pwd);
                /* also update _WORK environment variable used by path module */
                setstr(global("_WORK"), pwd);
	}
#endif
	else
		unset(v_pwd);
	...
	register char **wp;
{
	...
	for (wp++; *wp != NULL && **wp == '-'; wp++) {
		...
		if (*s == '-') { /* AMOEBA fix: echo "-- bye --" should work */
			break;
		}
		if (*s == '\0') {
			wp++;
