...
char **argv;
{
    ...
    oldalloc = 512;
    oldhunk = (char *) malloc(oldalloc);
    newalloc = 512;
    newhunk = (char *) malloc(newalloc);
    ...
    if (argc != 2) {
	fprintf(stderr,"Usage: cdiff -cN old new\n");
	exit(1);
    }
	...
	if (isdigit(*buff)) {
		...
		if (oldsize + len + 1 >= oldalloc) {
		    oldalloc *= 2;
		    oldhunk = (char *) realloc(oldhunk, oldalloc);
		}
		if (i >= oldmin) {
		    (void) strcpy(oldhunk+oldsize, oldmark);
		    oldwanted++;
		}
		else {
		    (void) strcpy(oldhunk+oldsize, "  ");
		}
		(void) strcpy(oldhunk+oldsize+2,line);
		oldsize += len;
	    }
	    preoldmax = oldmax;
	    preoldend = oldend;
		...
		if (newsize + len + 1 >= newalloc) {
		    newalloc *= 2;
		    newhunk = (char *) realloc(newhunk, newalloc);
		}
		if (i >= newmin) {
		    (void) strcpy(newhunk+newsize, newmark);
		    newwanted++;
		}
		else {
		    (void) strcpy(newhunk+newsize, "  ");
		}
		(void) strcpy(newhunk+newsize+2,line);
		newsize += len;
	    }
	    prenewmax = newmax;
	    prenewend = newend;
