...
dupflags:
		fprintf (stderr,
"%s: you must specify exactly one of the c, t, x, or d options\n", tar);
		describe();
		exit(EX_ARGSBAD);
	}
	exit(0);
...
name_gather()
{
	...
	if (f_sorted_names) {
		...
		if (p) {
			...
			strncpy(namebuf[0].name, p, (size_t)namebuf[0].length);
	...
	char	*name;			/* pointer to name */
{
	...
	if (!p) {
		fprintf(stderr,
			"%s: cannot allocate mem for namelist entry\n", tar);
		exit(EX_SYSTEM);
	}
