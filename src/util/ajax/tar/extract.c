/*	@(#)extract.c	1.2	93/10/07 16:11:55 */
/*
...
/* returns true if the name exists and is a directory */
int
exists_dir(name)
char *name;
{
...
extract_archive()
{
...
#ifdef _S_IFCHR
	case LF_CHR:
		hstat.st_mode |= _S_IFCHR;
		goto make_node;
#endif

#ifdef _S_IFBLK
	case LF_BLK:
		hstat.st_mode |= _S_IFBLK;
		goto make_node;
#endif
	...
	make_node:
#ifdef AMOEBA
		fprintf(stderr, "not extracted (not a file or directory)\n");
		break;
#else
	...
	again_dir:
	    if (!exists_dir(skipcrud+head->header.name)) {
