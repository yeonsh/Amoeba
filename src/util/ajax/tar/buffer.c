/*	@(#)buffer.c	1.3	93/10/07 16:11:28 */
/*
	...
	char *msg;
{
	...
	if (from != to) {
		...
		err = dup(from);
		if (err != to) {
			fprintf(stderr, "%s: cannot dup ", tar);
			perror(msg);
			exit(EX_SYSTEM);
		}
		(void) close(from);
...
progerror(err)
char *err;
/* print program name and call perror with argument err */
{
	...
	char *rem_host, *rem_file;
{
	...
	err = pipe(pipes);
	if (err < 0) {
		progerror ("cannot create pipe to child");
		exit(EX_SYSTEM);
	}
	...
	if (ar_reading) {
		...
		if (rem_host) {
			...
			if (STDIN != open("/dev/null", 0))
				progerror("Can't open /dev/null");
			sprintf(cmdbuf,
				"rsh '%s' dd '<%s' bs=%db",
			...
			if (err != 0) {
				progerror("can't fstat archive");
				exit(EX_SYSTEM);
			}
			if (S_ISREG(arstat.st_mode)) {
				execlp("compress", "compress", "-d", (char *)0);
				progerror("cannot exec compress");
			} else {
...
gotit:
	if (blocksize == 0) {
		fprintf(stderr, "%s: invalid value for blocksize\n", tar);
		exit(EX_ARGSBAD);
	}
	...
	if (!ar_block) {
		fprintf(stderr,
		"%s: could not allocate memory for blocking factor %d\n",
			tar, blocking);
		exit(EX_ARGSBAD);
	}
...
fl_write()
{
	...
	if (err < 0)
		...
		fprintf(stderr, "%s: %s: write failed, short %d bytes\n",
			tar, ar_file, blocksize - err);
	exit(EX_BADARCH);
}
		...
		if (child != -1) {
			...
			case 0:
				...
				if (TERM_VALUE(status) == (SIGPIPE + 128)) {
					...
					fprintf(stderr,
				  "%s: child returned status %d\n",
						tar, TERM_VALUE(status));
			case SIGPIPE:
				break;		/* This is OK. */

			default:
				fprintf(stderr,
				 "%s: child died with signal %d%s\n",
				 tar, TERM_SIGNAL(status),
