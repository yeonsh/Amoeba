...
#ifndef AMOEBA
  /* Amoeba does this automatically, as required by ANSI C */
  _cleanup();
#endif
  exit(n);
}
...
static  int      selected();
static  void	 command(), readout(), initget();
...
register char	*buf;	   /* where to send the input */
{
	...
	if (buf > linebuf+MAXBUF) { truncated(0); --buf; }
