...
{
  write(2, message, (int) strlen(message));
  if (arg != NIL_PTR)
	write(2, arg, (int) strlen(arg));
  write(2, ".\n", 2);
  if (quit)
	exit(1);
}
...
/* Loop until all files minus one are done */
  ...
  if (smallest != NIL_MERGE) { /* we're not ready yet */
	if (only_merge && uniq)
...
#else
  extern int brk();	/* does not return char* but success/failure! */
