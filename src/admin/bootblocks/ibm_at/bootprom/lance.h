...
typedef struct {
    ...
    int8	ib_padr[6];	/* physical 48bit Ether-address */
    uint16	ib_ladrf[4];	/* 64bit hashtable for "logical" addresses */
    uint16	ib_rdralow;	/* low 16 bits of Receiver Descr. Ring addr */
    uint8	ib_rdrahigh;	/* high 8 bits of Receiver Descr. Ring addr */
    uint8	ib_rlen;	/* upper 3 bits are 2log Rec. Ring Length */
    uint16	ib_tdralow;	/* low 16 bits of Transm. Descr. Ring addr */
    uint8	ib_tdrahigh;	/* high 8 bits of Transm. Descr. Ring addr */
    uint8	ib_tlen;	/* upper 3 bits are 2log Transm. Ring Length */
} initblock_t;
...
typedef struct {
    ...
    int8	rmd_hadr;	/* high 8 bits of bufaddr */
    int8	rmd_flags; 	/* see below */
    int16	rmd_bcnt;	/* two's complement of buffer byte count */
...
typedef struct {
    ...
    uint8	tmd_hadr;	/* high 8 bits of bufaddr */
    uint8	tmd_flags;	/* see below */
    int16	tmd_bcnt;	/* two's complement of buffer byte count */
