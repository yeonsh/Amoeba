/*	@(#)dp8390.h	1.2	94/04/06 11:31:35 */
/*
...
typedef struct {
    int		dc_reg;			/* dp8390 base address */
    uint32	dc_mem;			/* base memory */
    uint8	dc_tpsr;		/* transmit start page */
    uint8	dc_pstart;		/* receive start page */
    uint8	dc_pstop;		/* receive end page */
} dpconf_t;


/* dp8390 receive header */
typedef struct {
    uint8	dh_status;		/* copy of rsr */
    uint8	dh_next;		/* pointer to next packet */
    uint8	dh_rbcl;		/* receive byte count low */
    uint8	dh_rbch;		/* receive byte count high */
} dphdr_t;
